                ...
                {!showCostConfirm ? (
                    <div className="p-8">
                        ...
                        <form onSubmit={(e) => {
                            ...
                            <input type="text" required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none text-[15px] font-bold text-slate-800 focus:ring-2 focus:ring-indigo-500/10 placeholder:text-slate-300 mb-8" placeholder="e.g. Account Help" value={newTicketSubject} onChange={(e) => setNewTicketSubject(e.target.value)} autoFocus />
                            <button type="submit" className="w-full bg-[#4F46E5] text-white py-4 rounded-2xl text-[13px] font-bold shadow-xl shadow-indigo-100 hover:bg-indigo-700">Start Conversation</button>
                        </form>
                    </div>
                ) : (
                    <div className="p-8 text-center">
                        ...
                        <div className="flex gap-4">
                            <button onClick={() => setShowCostConfirm(false)} className="flex-1 bg-slate-50 py-4 rounded-2xl text-[13px] font-bold text-slate-400">Cancel</button>
                            <button onClick={handleCreateTicket} className="flex-1 bg-[#4F46E5] text-white py-4 rounded-2xl text-[13px] font-bold shadow-lg shadow-indigo-100">Confirm</button>
                        </div>
                    </div>
                )}
            </div>
